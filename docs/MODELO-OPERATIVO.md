...
Espejo del esquema Postgres aplicado en `polaria-wms-db` (migraciones `020`–`040`). El cliente Prisma vive en `prisma/schema.prisma` y se genera en `src/generated/prisma`.
...
### 031–040 — Alineación v1 / flujos (post POL-33)

| Migración | Modelos nuevos / ampliados |
|-----------|---------------------------|
| 031 | Enums: `BodegaTipo`, `EstadoSlot`, `EstadoLote`, `TipoCamion`, `EstadoProcesamiento`, `TipoAlerta`, `EstadoAlerta`, `TipoTarea`, `EstadoTarea`, `EstadoTareaCuenta`, `EstadoIntegracion`, `DestinoTipo`, `EstadoSolicitudBodega` |
| 032 | `SolicitudAltaBodega`; `Bodega` (+tipo, capacidad, creador); `AsignacionBodega` (+idRol, vigenteDesde) |
| 033 | Catálogos ampliados; `Producto` (+primario/secundario, conversión, cliente) |
| 034 | `RecepcionCompra`, `RecepcionCompraLinea`; `SolicitudCompra`/`OrdenCompra` ampliados |
| 035 | `SolicitudProcesamiento`, `RegistroMerma`; `OrdenTrabajo` ampliado |
| 036 | `Ubicacion.estadoSlot`; `Lote` ampliado; `WarehouseState.cantidadReservada` |
| 037 | `AlertaOperativa`, `TareaCola` |
| 038 | `OrdenVenta` (+comprador, planta, creador, bodegaDestino) |
| 039 | `ViajeTransporte.idCamion`; `EvidenciaTransporte` ampliada |
| 040 | `SolicitudIntegracion`, `TareaCuenta` |
...
| `BodegaTipo` | `bodega_tipo` |
| `EstadoSlot` | `estado_slot` |
| `EstadoLote` | `estado_lote` |
| `TipoCamion` | `tipo_camion` |
...
| `EstadoSolicitudBodega` | `estado_solicitud_bodega` |
| `EstadoTareaCuenta` | `estado_tarea_cuenta` |
| `EstadoIntegracion` | `estado_integracion` |
